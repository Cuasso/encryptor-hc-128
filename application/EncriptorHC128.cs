...
namespace application
{
    public class EncriptorHC128
    {
        ...
        public EncriptorHC128(byte[] key, byte[] iv)
        {
            _key = key;
            _iv = iv;
        }
        ...
        /// a-b mod 512
        /// </summary>
        private UInt32 MinMod512(UInt32 a, UInt32 b) => (a - b) % 512;


        private UInt32 F1(UInt32 x) => RollRight(x, 7) ^ RollRight(x, 18) ^ (x >> 3);

        private UInt32 F2(UInt32 x) => RollRight(x, 17) ^ RollRight(x, 19) ^ (x >> 10);

        private UInt32 G1(UInt32 x, UInt32 y, UInt32 z) => (RollRight(x, 10) ^ RollRight(z, 23)) + RollRight(y, 8);

        private UInt32 G2(UInt32 x, UInt32 y, UInt32 z) => (RollLeft(x, 10) ^ RollLeft(z, 23)) + RollLeft(y, 8);

        private UInt32 H1(UInt32 x) => _Q[(byte)x] + _Q[256 + (byte)(x >> 16)];

        private UInt32 H2(UInt32 x) => _P[(byte)x] + _P[256 + (byte)(x >> 16)];

        private UInt32 RollRight(UInt32 x, int n) => ((x >> n) ^ (x << (32 - n)));

        private UInt32 RollLeft(UInt32 x, int n) => ((x << n) ^ (x >> (32 - n)));
